...
const Service = () => {
    ...
    useEffect(() => {
        iconRef.current.forEach((element, index) => {
            if (openIndex === index) {
                gsap.to(
                    element,
                    {rotation: 45, duration: 0.3, ease: 'linear'}
                )
            } else {
                gsap.to(
                    element,
    ...
    return (
                ...
                {currentText?.map((section, index) => (
                    <div key={index} className={cls.text}>
                        <div onClick={() => toggleList(index)} className={cls.headListBlock}>
                            ...
                            >
                                <path
                                    d="M18.8,16l5.5-5.5c0.8-0.8,0.8-2,0-2.8l0,0C24,7.3,23.5,7,23,7c-0.5,0-1,0.2-1.4,0.6L16,13.2l-5.5-5.5
        ...
        C8,24.8,8.5,25,9,25c0.5,0,1-0.2,1.4-0.6l5.5-5.5l5.5,5.5c0.8,0.8,2.1,0.8,2.8,0c0.8-0.8,0.8-2.1,0-2.8L18.8,16z"
            ...
            {
                !isMobile && <div className={cls.imageContainer}>
                    {isAuth &&
                        <img className={cls.replace}
                             src="/src/shared/assets/images/icons/replace.svg"
                             alt=""
                             onClick={() => openModal('service3_s.webp')}
                        />
                    }
                    <img className={cls.image} src={`${API_URL}/images/main/service3_s.webp`} alt="work_photo"/>
                </div>
            }
