...
const Admin = () => {
    const {registration, isAuth, login, logout} = useAuthStore(state => state)
    const [isRegister, setRegister] = useState<boolean>(false)
    const {reset: resetLogin, clearErrors: clearErrorsLogin, register: registerLogin, handleSubmit: handleSubmitLogin} = useForm<Login>();
    const {reset: resetReg, clearErrors: clearErrorsReg, register: registerReg, handleSubmit: handleSubmitReg} = useForm<Registration>();
    ...
    const regFormClasses = classNames({
        [cls.formContainer]: isRegister,
        [cls.hidden]: !isRegister,
    })
    ...
    return (
        <div>
            ...
            <Modal isOpen={true}>
                <svg onClick={() => navigate('/')} className={cls.svg} width="50px" height="50px" viewBox="0 0 24 24"
                     fill="none"
                ...
                {isAuth ? <button onClick={() => logout("token")} className={cls.changeFormButton}>Выйти</button> :
                    <div className={loginFormClasses}>

                        <button className={cls.changeFormButton} onClick={handleReg}>Регистрация</button>

                        <form className={cls.form} onSubmit={handleSubmitLogin(onSubmitLogin)}>
                            <input
                                className={cls.input}
                                placeholder={'Email'}
                                type="text"
                            ...
                            />
                            {isErrorAuth && <div>Логин или пароль неверные</div>}
                            <button className={cls.submitButton}>Вход</button>
                        </form>

                    </div>
                }
                <div className={regFormClasses}>
                    <button className={cls.changeFormButton} onClick={handleLogin}>Логин</button>
                    <form onSubmit={handleSubmitReg(onSubmitReg)} className={cls.form} action="">
                        <input
                            className={cls.input}
                            placeholder={'Email'}
                            type="email"
                            {...registerReg('email', {required: 'Email is required'})}
                        />
                        <input
                            className={cls.input}
                            placeholder={'Пароль'}
                            type="password"
                            {...registerReg('password', {required: 'Password is required'})}
                        />
                        <input
                            className={cls.input}
                            placeholder={'Секретный ключ'}
                            type="text"
                            {...registerReg('secretKey', {required: 'Secret Key is required'})}
                        />
                        <button className={cls.submitButton}>Регистрация</button>
                    </form>
                </div>
